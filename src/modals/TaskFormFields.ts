...
export function renderTaskFormFields(container: HTMLElement, ctx: TaskFormFieldsContext): void {
    ...
    renderPropRow(
      ...
      () => {
        ...
        renderSelectControl({
          container: cell,
          value: ctx.parentId,
          options: [{ id: '', label: t('No parent') }, ...parents.map((t) => ({ id: t.id, label: t.title }))],
  ...
  renderPropRow(
    ...
    () => {
      ...
      renderSelectControl({
        container: cell,
        value: task.status,
        options: statuses.map((s) => ({ id: s.id, label: t(s.label), color: s.color, icon: s.icon || undefined })),
        onChange: (id) => {
          task.status = id
          rerender()
        }
  ...
  renderPropRow(
    grid,
    task.type === 'milestone' ? t('Date') : t('Due'),
    () => {
      const cell = createDiv('pm-prop-value')
      renderDateControl({
        container: cell,
        value: task.due,
        emptyLabel: t('Set due date'),
    ...
    renderPropRow(
      ...
      () => {
        ...
        renderSelectControl({
          ...
          options: REPEAT_OPTIONS.map((option) => ({ ...option, label: t(option.label) })),
          onChange: (id) => {
            if (id === 'none') {
    ...
    const depRow = renderPropRow(
      ...
      () => {
        ...
        renderMultiSelect({
          container: cell,
          search: true,
          addLabel: t('Add dependency'),
          addLabelMore: t('Add another'),
  ...
  if (task.type !== 'milestone' && task.progress === 0 && !shownExtras.has('progress')) {
    hidden.push({ id: 'progress', label: t('Progress'), icon: 'percent' })
  }
  if (!task.recurrence && !shownExtras.has('repeat')) {
    hidden.push({ id: 'repeat', label: t('Repeat'), icon: 'repeat' })
  }
  if (task.dependencies.length === 0 && !shownExtras.has('depends')) {
    hidden.push({ id: 'depends', label: t('Depends on'), icon: 'link-2' })
  }
  if (hidden.length > 0) {
  ...
  if (project.customFields.length > 0) {
    ...
    cfSection.createEl('h4', { text: t('Custom fields'), cls: 'pm-modal-section-title' })
