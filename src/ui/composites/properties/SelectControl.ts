...
export function renderSelectControl(opts: SelectControlOpts): void {
  ...
  trigger.createSpan({ cls: 'pm-prop-inline-label', text: selected?.label ?? t(opts.placeholder ?? 'Select') })
  ...
  trigger.addEventListener('click', () => {
    ...
    const searchInput = opts.search
      ? pop.contentEl.createEl('input', {
          cls: 'pm-pop-field',
          attr: { placeholder: t(opts.searchPlaceholder ?? 'Search…'), spellcheck: 'false' }
        })
      : null
